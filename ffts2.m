function Fq = ffts2(X,Y,V,Xq,Yq,method,window)
...
%         [W, index] =Interpolation_Weights(X,Xql,window);
        ...
        % Convolution
        WV  = bsxfun(@times, W,V);
        num = accumarray(index(:), WV(:),[length(Xql)*length(Yql),1]);

        % Sample density function
        dnum = accumarray(index(:), W(:),[length(Xql)*length(Yql),1]);
...
Vq2 = reshape(Vq,[length(Yql),length(Xql)]);

Fq2 = fftshift(fft2(ifftshift(Vq2)));
...
function F_trimmed = TrimFourier(F,num,numy)
    if(num == size(F,1))
        ...
        if(floor(num/2)==ceil(num/2))
            ...
            F_trimmed =F_trimmed.* (1i*sin(acx+acy) + cos(acx+acy));
        end
    end
end
